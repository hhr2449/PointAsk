...
export function WorkspaceControlHeader({ record, records, expanded, onToggle, onSwitch }: {
  ...
  const switchable = records.length > 1;
  return <header className="pointask-control-header">
    <div className="pointask-control-brand"><strong>PointAsk</strong>{switchable ? <select aria-label="切换 PointAsk 线程"
      value={record.pendingThread.id} onChange={(event) => onSwitch(event.target.value)}>
      {records.map((item) => <option key={item.pendingThread.id} value={item.pendingThread.id}>{item.localThread.displayId}</option>)}
    </select> : <span>{record.localThread.displayId}</span>}</div>
