...
const primaryLabels = {
  send: '发送追问', retry: '重试', attach_latest_return: '附加最新回答并返回',
  attach_selection_return: '附加所选内容并返回', return: '返回原文', retry_return: '重试返回',
};

export function WorkspaceStatusView({ source, question, state, selectionSummary, canChooseRounds, busy, error,
  onPrimary, onContinue, onChooseRounds, onClearSelection, onReturn, onMore }: {
  source: string; question: string; state: WorkspaceControlDerivedState; selectionSummary?: string; canChooseRounds: boolean;
  ...
  return <div className="pointask-control-view">
    ...
    <div className="pointask-control-actions">
      {state.primary && <button type="button" className="pointask-primary" disabled={busy} onClick={onPrimary}>{primaryLabels[state.primary]}</button>}
    ...
    <div className="pointask-control-low-frequency">
      {canChooseRounds && <button type="button" onClick={onChooseRounds}>选择其他轮次</button>}
