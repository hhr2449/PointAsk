...
export function isActiveWorkspaceThread(record: PendingAssociation, transient = false): boolean {
  ...
  if (rounds.some((round) => round.persistenceStatus === 'staged' || round.persistenceStatus === 'attaching' ||
    round.persistenceStatus === 'capture_failed')) return true;
  if (rounds.some((round) => ['waiting_for_submission', 'waiting_for_answer', 'generating', 'answer_ready', 'failed'].includes(round.status))) return true;
